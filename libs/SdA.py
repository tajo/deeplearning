"""
  This code is based on http://www.deeplearning.net libraries
  Modified by Vojtech Miksu
  3/2/2014
...
def plot_weights(layers, timestamp, dataset, pretraining_epochs, stage = ''):
  for i in xrange(0, len(layers)):
      stamp = datetime.datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d-%H-%M')
      image =  layers[i].W.get_value(borrow=True).T
      fig, ax = plt.subplots()
      ax.imshow(image, cmap=plt.cm.gray, interpolation='nearest')
      ax.set_xticklabels([])
      ax.set_yticklabels([])
      label = 'logs/pics/{}_{}_{}_layer_{}{}.png'.format(stamp, dataset, pretraining_epochs, i, stage)
      labelpdf = 'logs/pics/{}_{}_{}_layer_{}{}.pdf'.format(stamp, dataset, pretraining_epochs, i, stage)
      plt.savefig(labelpdf)


def test_SdA(finetune_lr=0.1,
             targetColumn='predictClassBi',
    ...
    stamp = datetime.datetime.fromtimestamp(start_time_global).strftime('%Y-%m-%d %H-%M')
    datasets = load_data('datasets/output-'+dataset+'-train.csv', 'datasets/output-'+dataset+'-valid.csv', 'datasets/output-'+dataset+'-test.csv', targetColumn, targets)
    train_set_x, train_set_y = datasets[0]
    valid_set_x, valid_set_y = datasets[1]
    test_set_x, test_set_y = datasets[2]
    ...
    plot_weights(sda.dA_layers, start_time_global, dataset, pretraining_epochs)
    ...
    with open(logfile, "a") as myfile:
      ...
      plot_weights(sda.dA_layers, start_time_global, dataset, pretraining_epochs, 'f')
